...
void SensorDataObserver::onFrameDataUpdated(CeleX5ProcessedData* pSensorData)
{
    ...
    unsigned char* pEvent_Buffer_1 = NULL;
    unsigned char* pEvent_Buffer_2 = NULL;
    unsigned char* pEvent_Buffer_3 = NULL;
    unsigned char* pEvent_Buffer_4 = NULL;
    int colorMode = 0;
    if (pSensorData->getSensorMode() == CeleX5::Event_Address_Only_Mode)
    {
        ...
        if (m_bSavePics)
        {
            pEvent_Buffer_1 = pSensorData->getEventPicBuffer(CeleX5::EventBinaryPic);
            pEvent_Buffer_2 = pSensorData->getEventPicBuffer(CeleX5::EventCountPic);
            pEvent_Buffer_3 = pSensorData->getEventPicBuffer(CeleX5::EventDenoisedBinaryPic);
            pEvent_Buffer_4 = pSensorData->getEventPicBuffer(CeleX5::EventDenoisedCountPic);
        }
    }
    ...
    else if (pSensorData->getSensorMode() == CeleX5::Event_Intensity_Mode)
    {
        ...
        if (m_bSavePics)
        {
            pEvent_Buffer_1 = pSensorData->getEventPicBuffer(CeleX5::EventBinaryPic);
            pEvent_Buffer_2 = pSensorData->getEventPicBuffer(CeleX5::EventCountPic);
            pEvent_Buffer_3 = pSensorData->getEventPicBuffer(CeleX5::EventGrayPic);
        }
    }
    ...
    if (m_bSavePics)
    {
        if (pSensorData->getSensorMode() == CeleX5::Event_Address_Only_Mode)
        {
            updateEventImage(pEvent_Buffer_1, 1);
            updateEventImage(pEvent_Buffer_2, 2);
            updateEventImage(pEvent_Buffer_3, 3);
            updateEventImage(pEvent_Buffer_4, 4);
        }
        else if (pSensorData->getSensorMode() == CeleX5::Event_Intensity_Mode)
        {
            updateEventImage(pEvent_Buffer_1, 1);
...
void CeleX5Widget::showPlaybackBoard(bool show)
{
    if (!m_pPlaybackBg)
    {
        ...
        m_pPlaybackBg->setGeometry(1350, 500, 550, 200);
