...
void SensorDataObserver::onFrameDataUpdated(CeleX5ProcessedData* pSensorData)
{
    ...
    else if (m_emDisplayType == ConvertBin2CSV)
    {
        ...
        cv::Mat mat = cv::Mat::zeros(cv::Size(CELEX5_COL, CELEX5_ROW), CV_8UC1);
        int dataSize = vecEvent.size();
        if(pSensorData->getSensorMode() == CeleX5::Event_Address_Only_Mode)
        {
            for (int i = 0; i < dataSize; i++)
            {
                mat.at<uchar>(CELEX5_ROW - vecEvent[i].row - 1, CELEX5_COL - vecEvent[i].col - 1) = 255;
                //cout << "-------------------" << "row = " << vecEvent[i].row << ", t = " << vecEvent[i].t << endl;
...
void SensorDataObserver::savePics()
{
    QDir dir;
    ...
    , m_emSensorFixedMode(CeleX5::Event_Address_Only_Mode)
{
    ...
    m_pFPSSlider->setDisplayName("Display FPS (ms)");
    m_pFPSSlider->setObjectName("Display FPS");
    //
    //
    //--- create comboBox to select image type
    QComboBox* pEventShowType = new QComboBox(this);
    pEventShowType->setGeometry(1350, 80, 250, 30);
    pEventShowType->show();
...
void CeleX5Widget::playback(QPushButton *pButton)
{
    if ("Playback" == pButton->text())
    {
        ...
        if (m_pCeleX5->openBinFile(filePath.toStdString()))
        {
            ...
            if(m_pCeleX5->isLoopModeEnabled())
            {
                switchMode(button[0],true);
            }
            else
            {
                switchMode(button[0],false);
    ...
    else
    {
        ...
        m_pCeleX5->setPlayBackState(false);
        if(!m_pCeleX5->isLoopModeEnabled())
        {
...
void CeleX5Widget::switchMode(QPushButton* pButton,bool isLoopMode)
{
    if (isLoopMode)
    {
        ...
        m_pSensorDataObserver->setPictureMode(0);
    }
    else
    {
        ...
        if (m_pSensorDataObserver->getDisplayType() == Realtime)
        {
            m_pBtnShowStyle->show();
            m_pCeleX5->setSensorFixedMode(m_emSensorFixedMode);
            m_pCbBoxImageType->setCurrentIndex((int)m_emSensorFixedMode);
        }
...
void CeleX5Widget::showPlaybackBoard(bool show)
{
    if (!m_pPlaybackBg)
    {
        ...
        m_pPlaybackBg->setGeometry(1350, 380, 550, 300);

        CfgSlider* pSlider2 = new CfgSlider(m_pPlaybackBg, 0, 1000, 1, 0, this);
        pSlider2->setGeometry(0, 10, 500, 70);
        ...
        m_pLabelCurrentTime->setGeometry(220, top-20, 70, 40);
        ...
        qsFontSize = "16px";
#endif
        m_pBtnSavePic = new QPushButton("Start Saving Bmp", m_pPlaybackBg);
        m_pBtnSavePic->setGeometry(100, 250, 150, 30);
        m_pBtnSavePic->setStyleSheet("QPushButton {background-color: #EEEEEE; font: "+qsFontSize+" Calibri; "
...
void CeleX5Widget::onBtnSavePicReleased()
{
    if (m_pSensorDataObserver->isSavingBmp())
    {
        ...
        m_pBtnSavePic->setText("Start Saving Bmp");
        m_pBtnSavePic->setStyleSheet("QPushButton {background-color: #EEEEEE; font: 18px Calibri; "
                                     "border-style: outset; border-width: 2px; border-radius: 10px; border-color: #FFFFFF;} "
                                     "QPushButton:pressed {background: #992F6F; }");
    }
    else
    {
        ...
        m_pBtnSavePic->setText("Stop Saving Bmp");
        m_pBtnSavePic->setStyleSheet("QPushButton {background-color: #992F6F; color: yellow; font: 18px Calibri; "
                                     "border-style: outset; border-width: 2px; border-radius: 10px; border-color: #FFFFFF;} "
                                     "QPushButton:pressed {background: #992F6F; }");
    }
}
