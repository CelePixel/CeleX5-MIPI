...
void CeleX5::getMIPIData(unsigned char* buffer, uint32_t* length)
{
	if (CeleX5::CeleX5_MIPI != m_emDeviceType)
	{
		return;
	}
	if (m_pCeleDriver->getPackage(buffer, length))
	{
		//record sensor data
		if (m_pDataRecorder->isRecording())
		{
			m_pDataRecorder->writeData(buffer, *length);
		}
		//cout << "image buffer size = " << buffer.size() << endl;
		//m_pDataProcessThread->addData(buffer);
...
void CeleX5::setSensorFixedMode(CeleX5Mode mode)
{
	...
	{
		if (m_uiLastClockRate != m_uiClockRate)
...
bool CeleX5::readBinFileData()
{
	...
	if (dataLen > 0)
	{
		m_pDataProcessThread->addData(m_pDataToRead, dataLen);
