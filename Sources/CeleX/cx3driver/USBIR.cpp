...
bool USBIR::video_start(void)
{
#ifdef __linux__
	if (pthread_create(&m_threadID, nullptr, staticThreadFunc, this) == 0)
	{
#else
	m_handle = CreateThread(nullptr, 0, staticThreadFunc, this, 0, NULL);
	if (m_handle)
	{
#endif
		return true;
...
#ifdef __linux__
void* USBIR::staticThreadFunc(void* args)
{
	USBIR* pThread = (USBIR*)args;
	pThread->Worker();
	return NULL;
}
#else
DWORD  USBIR::staticThreadFunc(LPVOID args)
{
	USBIR* pThread = (USBIR*)args;
	pThread->Worker();
	return 0;
