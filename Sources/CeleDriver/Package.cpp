...
bool CPackage::GetPackage(unsigned char* buffer, uint32_t* length)
{
	if (Status == BUFFER_STATUS_FULL)
	{
		int package_size = 0;
		for (size_t i = 0; i < Offset; i++)
		{
			int len = element_list[i]->end() - element_list[i]->begin();
			package_size += len;
			memcpy(buffer + package_size, element_list[i]->begin(), len);
		}
		//printf("--------------- %d\n", package_size);
		*length = package_size;
		Offset = 0;
