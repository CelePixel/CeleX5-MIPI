...
int main()
{
	...
	while (true)
	{
		...
		cv::Mat matOpticalSpeed = pCeleX->getOpticalFlowPicMat(CeleX5::Full_Optical_Flow_Speed_Pic);	//get optical flow speed buffer
		cv::imshow("Optical-Flow Speed Buffer - Gray", matOpticalSpeed);

		cv::Mat matOpticalDirection = pCeleX->getOpticalFlowPicMat(CeleX5::Full_Optical_Flow_Direction_Pic);	//get optical flow direction buffer
